...
DirName='Statistic_Data'
current_dir='/staging/reserve/aging/chia2831/FabryDisease/Merge_Fabry_total_70/Statistic_Data/'

## Check for dir, if not found create it using the mkdir ##
create_dir="/staging/reserve/aging/chia2831/FabryDisease/Merge_Fabry_total_70/Reads_info/"
[ ! -d "$create_dir" ] && mkdir -p "$create_dir"

vcfgz_path='/staging/reserve/aging/chia2831/FabryDisease/Output/hg38/VCF/'
recode_vcf_path='/staging/reserve/aging/chia2831/FabryDisease/Merge_Fabry_total_70/recode_vcf/'
...
while IFS='' read -r line || [[ -n "$line" ]]; do
	...
	Total_Yield=`awk 'NR==24' ${current_dir}${line}.stats`
	echo -e "$line\t$Total_Yield" >> ${output_path}2_Total_Yield.txt

	## CoverageDist
	grep '^COV' ${current_dir}${line}.stats | cut -f 2- > ${output_path}grep_${line}_stats_COV_all.txt
	### Reads <= 9 Depth
	...
	# flagstat
	## grep 'in total'
	RawData_Reads_1=`awk 'NR==1' ${current_dir}${line}.flagstat`
	RawData_Reads_2=`awk 'NR==2' ${current_dir}${line}.flagstat`
	...
	## grep 'duplicates'
	Duplicates_Reads_PassQC=`awk 'NR==5' ${current_dir}${line}.flagstat`
	echo -e "$line\t$Duplicates_Reads_PassQC" >> ${output_path}2_Duplicates_Reads_PassQC.txt

	## grep 'mapped'
	Mappable_Reads_PassQC=`awk 'NR==7' ${current_dir}${line}.flagstat`
	...
	# depth
	AvgDepth=`awk 'NR==1' ${current_dir}${line}.avg_depth`
	Stdev=`awk 'NR==2' ${current_dir}${line}.avg_depth`
