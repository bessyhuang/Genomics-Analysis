...
VCF_dir='/staging2/reserve/flagship/chia2831/TEST_2000_genome_VCFgz/VCF/'
...
while IFS='' read -r line || [[ -n "$line" ]]; do
	# Check file exist or not
        echo "==> $line"
        echo -e "`ls ${VCF_dir} | grep ${line}`"

	
	# Modify : --vcf
	/staging/reserve/aging/chia2831/bin/vcftools --vcf "${VCF_dir}${line}".vcf --recode --recode-INFO-all \
